...
from pytorch_trainer.training import extension as extension_module
from pytorch_trainer.training import trigger as trigger_module
from pytorch_trainer.utils import argument
...
class Trainer(object):
    ...
    - The order of execution of extensions is determined by their priorities.
      ...
      - ``PRIORITY_WRITER``. This is the priority for extensions that write
        ...
        or the extension uses the :func:`pytorch_trainer.report` function to report
    ...
    The :meth:`~pytorch_trainer.training.extensions.snapshot` method makes regular
    snapshots of the :class:`~pytorch_trainer.training.Trainer` object during training.
    ...
    During the training, it also creates a :class:`~pytorch_trainer.Reporter` object to
    ...
    :meth:`pytorch_trainer.Link.namedlink` method, which represents the path to each
    ...
    Args:
        updater (~pytorch_trainer.training.Updater): Updater object. It defines how to
            update the models.
    ...
    def run(self, show_loop_exception_msg=True):
        ...
        try:
            ...
            if show_loop_exception_msg:
                # Show the exception here, as it will appear as if pytorch_trainer
