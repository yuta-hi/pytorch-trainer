...
device=None, eval_hook=None, eval_func=None, *, progress_bar=False)
    ...
    It creates a :class:`~pytorch_trainer.Reporter` object to store values observed in
    the evaluation function on each iteration. The report for all iterations
    are aggregated to :class:`~pytorch_trainer.DictSummary`. The collected mean values
    ...
    :class:`~pytorch_trainer.Reporter` for details in naming rules of the reports.
    ...
    (i.e., ``pytorch_trainer.config.train`` is set to ``False``).
    ...
    Args:
        ...
        converter: Converter function to build input arrays.
            :func:`~pytorch_trainer.dataset.concat_examples` is used by default.
        ...
        progress_bar: Boolean flag to show a progress bar while training,
            which is similar to
            :class:`~pytorch_trainer.training.extensions.ProgressBar`.
            (default: ``False``)
